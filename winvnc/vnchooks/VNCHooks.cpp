...
KillPropsProc(HWND hwnd, LPARAM lParam)
{
	...
	RemoveProp(hwnd, (LPCTSTR)(uintptr_t) MAKELONG(VNC_POPUPSELN_ATOM, 0));
	return TRUE;
}
	...
	case 485:  // HACK to handle popup menus
		{
			...
			HANDLE prop = GetProp(hWnd, (LPCTSTR)(uintptr_t) MAKELONG(VNC_POPUPSELN_ATOM, 0));
			if (prop != (HANDLE) wParam)
			{
				// It did, so update the menu & the selection value
				SendDeferredWindowRect(hWnd);
				SetProp(hWnd,
					(LPCTSTR)(uintptr_t) MAKELONG(VNC_POPUPSELN_ATOM, 0),
					(HANDLE) wParam);
			}
		}
		break;
