	...
	if (!g_bEffectsDisabled) {
		...
		for (iParam = 0; iParam < (sizeof(spiParams) / sizeof(spiParams[0])); iParam++) {
			if (spiValues[iParam] != spiSuggested[iParam]) {
				LONG_PTR value = spiSuggested[iParam];
				if (!SystemParametersInfo(spiParams[iParam]+1, 0, (PVOID)value, SPIF_SENDCHANGE)) {
	...
	if (g_bEffectsDisabled) {
		...
		for (iParam = 0; iParam < (sizeof(spiParams) / sizeof(spiParams[0])); iParam++) {
			if (spiValues[iParam] != (BOOL) spiSuggested[iParam]) {
				LONG_PTR value = spiValues[iParam];
				if (!SystemParametersInfo(spiParams[iParam]+1, 0, (PVOID)value, SPIF_SENDCHANGE)) {
	...
	if (g_bDisabledFontSmoothing && g_bGotOldFontSmoothingValue) {
		if (g_bOldFontSmoothingValue != FALSE) {
			if (!SystemParametersInfo(SPI_SETFONTSMOOTHING, g_bOldFontSmoothingValue, NULL, SPIF_SENDCHANGE)) {				
				...
				if (g_bGotClearType) {
					LONG_PTR value = g_bOldClearTypeValue;
					if (!SystemParametersInfo(0x1049 /*SPI_SETCLEARTYPE*/, 0, (PVOID)value, SPIF_SENDCHANGE)) {
				...
				if (g_bGotFontSmoothingType) {
					LONG_PTR value = g_nOldFontSmoothingType;
					if (!SystemParametersInfo(0x200B /*SPI_SETFONTSMOOTHINGTYPE*/, 0, (PVOID)value, SPIF_SENDCHANGE)) {
